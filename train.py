    ...
    try:
        for epoch in range(num_epochs):
            ...
            for datum in data_loader:
                ...
                if iteration == 0 :
                    best_map = 0
                    yolact_net.save_weights(save_path(epoch, repr(iteration) + '_best'))
                    print('Saving first iter, iter:', iteration)                    
            ...
            if args.validation_epoch > 0:
                if epoch % args.validation_epoch == 0 and epoch > 0:
                    ...
                    if best_map < temp_map:
                        best_map = temp_map
                        SavePath.remove_best(args.save_folder)
                        yolact_net.save_weights(save_path(epoch, repr(iteration) + '_best'))
                        print('Saving new best iter, iter:', iteration)                  
...
def compute_validation_map(epoch, iteration, yolact_net, dataset, flg=False , log:Log=None):
    with torch.no_grad():
        yolact_net.eval()
        ...
        if not flg:
            ...
            val_info,bst_map = eval_script.evaluate(yolact_net, dataset, train_mode=True , flag=flg)
            end = time.time()
